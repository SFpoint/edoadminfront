<template>
  ...
  <v-container :class="{'px-0': $vuetify.breakpoint.smAndDown}">
    <v-row class="my-4 mx-auto" style="max-width: 540px">

      <v-col cols="12">
        <h1 class="text-center accent--text mb-0 px-5">Восстановление пароля</h1>
      </v-col>
        ...
        <v-card outlined elevation="2" class="pt-4 pb-8" :class="isMobile ? 'px-6' : 'px-8'">
          ...
          <v-tabs-items v-model="regTab" class="pt-6">
            <v-tab-item v-for="(option, i) in regOptions" :key="i">
              <div v-if="i === 0">
                <!-- первый этап восстановления пароля, отправка почты -->
                <v-row class="mx-0" v-if="regType === 1">
                  ...
                  <v-col cols="6" class="pa-0 mt-4" v-if="!isStartFlag">
                    <v-btn depressed color="primary" class="mr-2" :disabled="$v.$invalid" @click="startReset">
                      Отправить
                    </v-btn>
                  </v-col>

                  <v-col cols="12" class="pa-0 mt-4" v-if="showEmailConfirmationAlert">
                    <v-alert type="info" outlined class="body-2">
                      На Ваш почтовый ящик отправлено письмо, содержащее ссылку для подтверждения нового пароля
                    </v-alert>
                  </v-col>
              ...
              <!-- второй этап восстановления пароля, ввод и подтверждение пароля -->
              <PasswordCreate
                  v-if="regType === 2"
                  :isMobile="isMobile"
...
export default {
  ...
  created() {
    if (this.$route.name === 'forget') {
      this.regType = 1
    } else if (this.$route.name === 'forgetConfirm') {
      this.regType = 2
  ...
  methods: {
    ...
    async requestCodeToPhone() {
      try {
        await authApi.startReset(this.phoneToSend);
        clearInterval(this.confirmationCodeTimer);
